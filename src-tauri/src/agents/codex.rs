...
mod tests {
    ...
    fn parse_output_parses_json_message() {
        ...
        let event = adapter
            .parse_output(r#"{"text":"hello from codex"}"#)
            .expect("expected parsed event");

        assert!(matches!(event, AgentEvent::Message { content } if content == "hello from codex"));
    }
    ...
    fn parse_output_parses_usage_payload() {
        let adapter = CodexAdapter;
        let event = adapter
            .parse_output(r#"{"usage":{"input_tokens":20,"output_tokens":10,"total_tokens":30,"cost_usd":0.02}}"#)
            .expect("expected parsed event");
